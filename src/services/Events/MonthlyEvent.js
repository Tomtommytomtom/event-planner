...
export default class MonthlyEvent extends Event {
  ...
  getNextDaysForLastWeekday() {
    const start = DateArithmetic.getDateofLastWeekdayInMonth(
      this.getWeekday(),
      this.getNextMonth()
    )
    return {
  ...
  getNextDaysForNthWeekday() {
    const eventStart = DateArithmetic.getNthWeekdayOfMonth(
      ...this.getNthWeekday(),
      this.getNextMonth()
    ) //ugh, thats ugly
    const eventEnd = DateArithmetic.addDaysToDate(
      eventStart,
      this.getDurationOfEvent()
    )
